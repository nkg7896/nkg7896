
<h1 align="center">Hi 👋, I'm Nicholas Guevara</h1>
<h3 align="center">🚀 Full Stack Developer • Cloud & Data Specialist • CDP Expert</h3>

<p align="center">
  <a href="https://www.linkedin.com/in/nkguevara/">
    <img src="https://img.shields.io/badge/LinkedIn-Nicholas%20Guevara-0A66C2?style=for-the-badge&logo=linkedin&logoColor=white"/>
  </a>
  <a href="https://raw.githubusercontent.com/nkg7896/nkg7896/main/ng-resume.pdf">
    <img src="https://img.shields.io/badge/Download%20Resume-000000?style=for-the-badge&logo=LibreOffice&logoColor=white"/>
  </a>
</p>

---

## About Me

```text
Building scalable systems ⚙️ | Working with data 📊 | Exploring AI 🤖
````

* 💡 I design and build **cloud-native, data-driven applications**
* 📊 Specialized in **Customer Data Platforms (CDPs) & identity resolution**
* ☁️ Strong experience with **Azure & AWS ecosystems**
* 🤖 Exploring **LLM workflows and AI-powered systems**

---

## Tech Stack

### Core

<p>
  <img src="https://skillicons.dev/icons?i=nodejs,express,react,dotnet" />
</p>

---

### ☁️ Cloud & Infrastructure

<p>
  <img src="https://skillicons.dev/icons?i=aws,azure" />
</p>

* API Gateway • Lambda • S3
* Serverless Architectures

---

### 📊 Data & CDP Ecosystem

```text
Adobe Experience Platform (AEP) • RTCDP • Sitecore CDP • Treasure Data CDP • Snowflake • NoSQL
```

---

### 📈 Analytics & Marketing

* Google Tag Manager • Marketo

---

### 🤖 AI / Emerging Tech

```text
Azure PromptFlow • LLM Workflows
```

---

### 🔧 DevOps & Tooling

<p>
  <img src="https://skillicons.dev/icons?i=git,postman" />
</p>

* CI/CD Pipelines
* Unit & Mutation Testing
* AWS CloudWatch
* CloudWatch Logs Insights

---

### 🌐 Networking

```text
Infoblox (DNS) • WLAN • Network Configuration • Data Center Migration
```

---

## 📊 GitHub Activity

<p align="center">
  <img src="https://github-readme-streak-stats.herokuapp.com?user=nkg7896&theme=tokyonight&hide_border=true" />
</p>

---

## 🏅 Certifications

* Adobe Real-Time CDP
* Sitecore 10 (.NET)
* Treasure Data CDP

---

## What I Focus On

```text
• Scalable Backend Systems
• Customer Data Platforms (CDP)
• Identity Resolution
• Cloud-native & Serverless Architectures
• AI-powered Workflows
```

---

---

## In-Focus: Virtual Homelab

```text
Designing a virtual homelab to simulate real-world infrastructure and security scenarios
````

### Lab Stack

* 🖥️ Virtualization host for managing VMs and containers
* 🐳 Docker-based services for self-hosted apps and automation
* 💾 Separate SSD-backed storage for performance and isolation
* 🌐 Networking, DNS, and backup workflows for resilience

---

### What I’m Building

* A repeatable lab environment for **Red Teaming practice**
* A small service ecosystem that can be rebuilt from scratch
* Structured documentation covering setup, architecture, and learnings

---

### Why It Matters

This project gives me a practical environment to continuously learn and experiment with **virtualization, storage, networking, and automation** — while building a system I genuinely enjoy working on.



---




## Let's Connect

<p align="center">
  <a href="https://www.linkedin.com/in/nkguevara/">
    <img src="https://img.shields.io/badge/LinkedIn-Nicholas%20Guevara-blue?style=for-the-badge&logo=linkedin" />
  </a>
</p>

---


### Random Dev Quote
![](https://quotes-github-readme.vercel.app/api?type=horizontal&theme=radical)


---

<p align="center">
  ⭐️ Open to collaborations, interesting projects, and new ideas
</p>
<p align="center">
  <img src="https://komarev.com/ghpvc/?username=nkg7896&label=Visitors&color=blueviolet&style=for-the-badge" />
</p>

